...
- 容器编排：Kubernetes 1.28, Calico CNI
- 自动化工具：Helm，kubeadm, kubectl
- 脚本开发：Python, Shell
- CI/CD: GitLab（模拟实现）
- 容器运行时：Docker
- 监控运维：监控脚本
...
```text
Kubernetes（常简称为K8s首次发布2014年6月7日）是用于自动部署、扩展和管理“容器化（containerized）应用程序”的开源系统。该系统由Google设计并捐赠给Cloud Native Computing Foundation（今属Linux基金会）来使用。它旨在提供“跨主机集群的自动部署、扩展以及运行应用程序容器的平台”。它支持一系列容器工具，包括Docker等。（来源WiKi）
Calico is a single platform for networking, network security, and observability for any Kubernetes distribution in the cloud, on-premises, or at the edge. Whether you're just starting with Kubernetes or operating at scale, Calico's open source, enterprise, and cloud editions provide the networking, security, and observability you need.（来源calico官网about）
Helm is an open source package manager for Kubernetes. It provides the ability to provide, share, and use software built for Kubernetes.(来源CNCF Helm Project Journey report)
Kubeadm 是一个提供了 kubeadm init 和 kubeadm join 的工具，作为创建Kubernetes集群的“快捷途径”的最佳实践。kubeadm 通过执行必要的操作来启动和运行最小可用集群。按照设计，它只关注启动引导，而非配置机器。同样的，安装各种“锦上添花”的扩展，例如 Kubernetes Dashboard、监控方案、以及特定云平台的扩展，都不在讨论范围内。相反，我们希望在kubeadm之上构建更高级别以及更加合规的工具，理想情况下，使用 kubeadm 作为所有部署工作的基准将会更加易于创建一致性集群。(来源Kubernetes官网文档)
Kubernetes 提供 kubectl 是使用 Kubernetes API 与 Kubernetes 集群的控制面进行通信的命令行工具。这个工具叫做 kubectl。(来源Kubernetes官网文档)
Docker是一个开放源代码的开放平台软件，用于开发应用、交付（shipping）应用和运行应用。Docker允许用户将基础设施（Infrastructure）中的应用单独分割出来，形成更小的颗粒（容器），从而提高交付软件的速度。Docker容器与虚拟机类似，但二者在原理上不同。容器是将操作系统层虚拟化，虚拟机则是虚拟化硬件，因此容器更具有便携性、更能高效地利用服务器。 容器更多的用于表示软件的一个标准化单元。由于容器的标准化，因此它可以无视基础设施（Infrastructure）的差异，部署到任何一个地方。另外，Docker也为容器提供更强的业界的隔离兼容。Docker 利用Linux核心中的资源分离机制，例如cgroups，以及Linux核心命名空间（namespaces），来建立独立的容器（containers）。这可以在单一Linux实体下运作，避免启动一个虚拟机造成的额外负担。Linux核心对命名空间的支持完全隔离了工作环境中应用程序的视野，包括行程树、网络、用户ID与挂载文件系统，而核心的cgroup提供资源隔离，包括CPU、存储器、block I/O与网络。从0.9版本起，Dockers在使用抽象虚拟是经由libvirt的LXC与systemd - nspawn提供界面的基础上，开始包括libcontainer函数库做为以自己的方式开始直接使用由Linux核心提供的虚拟化的设施。（来源WiKi）
```
...
2. 基础环境设置
   ...
   # 关闭swap
   systemctl disable dev-sda3.swap
   ...
   # 设置主机名（分别在各自节点执行）
   hostnamectl set-hostname k8s-master   # 在master节点
   hostnamectl set-hostname k8s-node1    # 在node1节点
   hostnamectl set-hostname k8s-node2    # 在node2节点
   ...
   对以上操作的一些解释
   # 关闭swap
   当我发现只用swapoff -a 和 sed -i '/ swap / s/^\(.*\)$/#\1/g' /etc/fstab注释掉有关swap的行重启VMware虚拟机后仍然可以用free -h看到swap未被禁用这是由于Ubuntu在启动时检测到swap分区并自动启用，即使fstab中没有条目。
   sudo systemctl stop dev-sda3.swap   # 停止具体 swap 设备服务(如果之前没设置，重启后需要用这一条)
   sudo systemctl disable dev-sda3.swap # 禁用开机启动
   sudo systemctl mask dev-sda3.swap    # 屏蔽服务（无法被意外激活）
   sudo systemctl mask swap.target      # 屏蔽整个 swap 目标
   
   # 如果需要恢复 swap
   sudo systemctl unmask swap.target
   sudo systemctl unmask dev-sda3.swap
   sudo systemctl enable dev-sda3.swap
   sudo sed -i 's/^#\(.* swap .*\)$/\1/g' /etc/fstab
   sudo swapon -a
   
   # 解释: sed -i '/ swap / s/^\(.*\)$/#\1/g' /etc/fstab
   1. sed -i (-i选项：直接修改原文件)
   2. / swap / (地址定位：只匹配包含"swap"（前后有空格）的行)
   3. s/^\(.*\)$/#\1/g 
    - s/pattern/replacement/flags: 替换命令
    - ^\(.*\)$:正则表达式 ^行首  \(.*\)捕获整行内容到分组\1  $行尾
    - #\1: 替换内容，在捕获的内容前加#注释符
    - g: 全局替换标志
   ...
   作用：支持OverlayFS文件系统
   modprobe overlay
   容器镜像分层： Docker/Containerd使用OverlayFS实现镜像的分层存储
   写时复制：多个容器可以共享基础镜像层， 只在写入时创建新层
   性能优化： 减少磁盘空间占用，加速容器启动
   
   br_netfilter模块
   2. br_netfilter模块
   modprobe br_netfilter
   桥接网络流量过滤：让Linux网桥能够处理iptables/ip6tables规则
   Kubernetes网络策略： 支持NetworkPolicy对桥接网络流量的控制
   服务发现：确保Service的ClusterIP在节点内可访问
   ...
   net.bridge.bridge-nf-call-ip6tables = 1
   作用：
   让经过Linux网桥的网络数据包也经过iptables规则
   确保Kubernetes Service的负载均衡正常工作
   支持NetworkPolicy对Pod网络流量的控制
   如果不设置：
   Service的ClusterIP在节点内无法访问
   kube-proxy的iptables规则对桥接流量失效
   NetworkPolicy无法正确执行
   2. IP转发
   net.ipv4.ip_forward = 1
   作用:
   允许Linux节点作为路由器转发IP数据包
   Pod跨节点通信的必要条件
   容器网络插件(CNI)的基础条件
   为什么默认不开启这些设置？
   1. 安全考虑
   # 默认关闭IP转发是为了安全
   net.ipv4.ip_forward = 0  # 默认值
   减少攻击面： 不转发IP包可以防止主机意外成为路由器
   网络隔离： 默认情况下主机应该只处理自动的网络流量
   安全最佳实践：“最小权限原则”在网络层的体现
   2. 性能考虑
   内核模块按需加载：减少内存占用和启动时间
   避免不必要的开销：不是所有系统都需要桥接网络功能
   资源优化： 只为实际需要的功能分配内核资源
   3. 兼容性和稳定性
   模块冲突：某些模块可能与特定硬件或驱动不兼容
   避免功能冗余： 不同网络方案可能需要不同的内核配置
...
2. #### 安装Docker
   ...
   **所有节点执行：**
   ...
   # Step 6: 配置docker镜像加速（限时，可能需要登录或付费）
   # 一键安装配置脚本
   ## 轩辕镜像
...
3. #### 安装kubeadm, kubelet, kubectl
   ...
   **所有节点执行：**
   ...
   ## 访问地址：https://developer.aliyun.com/mirror/kubernetes?spm=a2c6h.13651102.0.0.5be51b11nfae9D
   
   
   # 由于containerd的CRI插件默认关闭，需要打开
   ...
   ####
   # 优化crictl (让crictl按containerd的socket工作)
...
1. #### Master节点初始化
   ...
   **仅在master节点执行：**
   ...
   ### 需要将calico.yaml里的源替换为国内源，修改后的calico.yaml如下
   kubectl apply -f calico.yaml
   
   ### 关于国内calico源
   访问地址： https://docker.aityp.com/r/docker.io/calico
   ```
...
2. #### Worker节点加入
   ...
   **在node1和node2节点执行：**
   ...
   ```bash
   # 使用master初始化时输出的join命令，类似：
   kubeadm join 192.168.100.10:6443 --token xxxxxx \
       --discovery-token-ca-cert-hash sha256:xxxxxx
   
   ### 关于上述命令的一些解释
   初始化(kubeadm init)的时候,输出中会直接显示完整的 join 命令
   ...
   # 方法二：使用 kubeadm
   kubeadm certs certificate-hash
   
   
   # 最简单的方法，一键生成完整的join命令
...
*由于国内Helm源大多都比较旧，没有找到比较好的Helm源，只找到Helm Chart相关的源，所以这个项目不做Helm的应用*
...
1. #### 节点监控脚本
   ...
   ```bash
   # 安装Metrics Server，未安装Metrics Server会导致无法使用kubectl top命令
   wget https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml
   vim components.yaml
   # 找到image: swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/metrics-server/metrics-server:v0.8.0改成国内源
   # 找到- --secure-port=10250 改为- --secure-port=4443
   # 找到- --kubelet-use-node-status-port删掉 添加- --kubelet-insecure-tls
   # 找到- - containerPort: 10250 改为- - containerPort: 4443
   kubectl apply -f components.yaml
   # 验证是否安装成功
   kubectl top nodes
   kubectl top pods --all-namespaces
   
   ### 关于修改components.yaml的一些解释
   kubelet 默认监听在 TCP 10250 端口上，metrics-server 会通过 HTTPS 主动访问各节点的 kubelet 以获取资源使用数据。若 kubelet 的 TLS 证书与节点 IP / hostname 不匹配，metrics-server 在访问 kubelet 时会发生 x509 校验错误，导致内部 collector 出错，从而使 /readyz 探针返回 500，Pod 处于 NotReady 状态。
   --kubelet-insecure-tls 的作用是让 metrics-server 在访问 kubelet 的 HTTPS 时跳过证书校验，从而避免上述 TLS 错误。
   --kubelet-use-node-status-port 参数会让 metrics-server 从 Node 对象的 Status.DaemonEndpoints.KubeletEndpoint.Port 读取 kubelet 端口。但在 kubeadm 默认环境中，Node 对象往往没有正确填入此字段。删除该参数可让 metrics-server 使用默认的 kubelet 端口 10250，从而确保访问正常。
   ...
   **小tips**
   ...
   寻找相关字段，解析json结构
   apt install jq -y
   jq 是处理 JSON 的小宝刀，拿在手里，Kubernetes、Docker、Prometheus 这些动不动就丢给你一坨 JSON 的世界立刻变得干净利落。它的风格有一点像“给 JSON 写 SQL”，但语法简单又犀利，很适合脚本、排查故障、临时过滤数据。
   最常用的三种取值方式：点号、方括号、管道。
   如果输入是一个对象，那么 jq '.' 会把它打印成漂亮的格式。
   如果你想拿其中一个字段，比如要 phase，就写 '.status.phase'。 jq 会从 JSON 结构里顺着路径把那段数据拎出去。
   如果输入是一组 items，那么 '.items[0]' 拿第一个，'.items[].metadata.name' 拿出所有名字。
   这些操作就像你在读取一个 JSON 文件，语法干净利落。
   过滤也很直观，比如 '.items[] | select(.status.phase != "Running")' 会找到所有不是 Running 的 Pod。 jq 会遍历 items 里的每一项，然后按条件筛选，输出符合条件的那几个。和 SQL 的 WHERE 有点像，只是写法更像数学表达式。
   你要是遇到数组，还可以按照值来挑选：'[.items[].status.phase] | unique'。方括号会把选出来的东西放到一个新的数组里，后面再接 unique 就能看到有哪些状态。
   格式化输出也很常用，比如 "--raw-output" 会去掉引号，把内容变成纯文本，这样脚本里读起来不啰嗦。
   如果你想类型保持原样，不要被当成字符串，也可以用 '--compact-output'，它会把 JSON 压成一行，适合进一步处理。
   jq 还有一个非常贴心的地方：不会报一大堆无意义的错。如果你访问不存在的字段，比如 .foo.bar，它不会炸，只会输出 null。写脚本的时候很省心，不需要加一堆防御性判断。
   ```
       ...
       try:
           ...
           for node in nodes_data['items']:
               ...
               node_status.append({
                   'name': name,
                   'status': status,
                   'role': 'master' if 'master' in name else 'worker'
               })
   ...
   def send_alert(message):
       ...
       # 实际环境中可以集成邮件、钉钉、企业微信等
   
   def main():
       """主监控循环"""
       while True:
           print(f"\n=== Cluster Status Check at {time.strftime('%Y-%m-%d %H:%M:%S')} ===")
           ...
           for node in nodes:
               ...
               if node['status'] != 'True':
                   send_alert(f"Node {node['name']} is not healthy!")
...
2. #### 自动扩缩容脚本
   ...
   class K8sAutoScaler:
       def __init__(self, deployment_name, namespace="default", max_replicas=10, min_replicas=1, cpu_threshold=80):
       ...
       def get_cpu_usage(self):
           ...
           try:
               ...
               for line in lines:
                   if self.deployment_name in line:
                       parts = line.split()
                       if len(parts) >= 3:
                           cpu_str = parts[1]
                           if 'm' in cpu_str:  # 转换为毫核
                               cpu = int(cpu_str.replace('m', ''))
                           else:
                               cpu = int(float(cpu_str) * 1000)  # 核心转毫核
                           total_cpu += cpu
                           pod_count += 1
       ...
       def run(self):
           ...
           while True:
               try:
                   ...
                   print(f"Current: {current_replicas} replicas, CPU: {cpu_usage}m")
                   
                   # 扩缩容逻辑
                   if cpu_usage > self.cpu_threshold * 10:  # 转换为毫核比较
                       if current_replicas < self.max_replicas:
                           new_replicas = current_replicas + 1
                           self.scale_deployment(new_replicas)
                   elif cpu_usage < (self.cpu_threshold * 10) / 2:
   ...
   if __name__ == "__main__":
       ...
       scaler = K8sAutoScaler(
           deployment_name="my-wordpress",  # 替换为你的部署名
           namespace="default",
           max_replicas=5,
           min_replicas=1,
           cpu_threshold=80
...
```bash
# 创建 ci-cd-simulator.sh
touch ci-cd-simulator.sh
vim ci-cd-simulator.sh

### 复制以下内容到ci-cd-simulator.sh
...
# 等待部署完成
echo "⏳ 等待应用部署完成..."
kubectl rollout status deployment/ci-test-app

# 阶段3: 验证部署
echo ""
echo "✅ 阶段3: 验证部署"
kubectl get deployments
kubectl get pods -l app=ci-test-app
kubectl get services

echo ""
echo "🎉 CI/CD 流水线执行完成!"
echo "测试应用已部署，可以通过以下命令访问:"
echo "kubectl port-forward deployment/ci-test-app 8080:80"

###

# 给脚本执行权限
chmod +x ~/ci-cd-simulator.sh
...
1. #### 集群基础设施
   ...
   - 使用kubeadm部署3节点高可用K8s集群
   - 配置Calico网络插件实现Pod网络通信
   - 解决国内环境下的镜像拉取和网络配置问题

2. #### 自动化运维

   - 简单开发Python集群健康监控脚本，实时监控节点状态
   - 编写自动化备份脚本，支持集群配置备份
   - 实现资源使用报告生成功能
